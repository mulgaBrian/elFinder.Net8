...
namespace elFinder.Net.Core.Services.Drawing
{
    public interface IPictureEditor
    {
        ...
        Task<ImageWithMimeType> CropAsync(Image input, int x, int y, int width, int height, int? quality = null);
        ...
        Task<ImageWithMimeType> ScaleAsync(Image input, int width, int height, int? quality = null);
    }
}