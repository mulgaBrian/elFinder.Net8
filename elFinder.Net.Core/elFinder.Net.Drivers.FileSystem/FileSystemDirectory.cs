...
namespace elFinder.Net.Drivers.FileSystem
{
  public class FileSystemDirectory : IDirectory
  {
    ...
    public virtual async Task DeleteAsync(bool verify = true, CancellationToken cancellationToken = default)
    {
      ...
      foreach (IFile file in (await GetFilesAsync(verify: false, _ => true, cancellationToken: cancellationToken)))
      {
        await file.DeleteAsync(verify, cancellationToken);
      }

      foreach (IDirectory dir in await GetDirectoriesAsync(verify: false, _ => true, cancellationToken: cancellationToken))
        ...
        Func<IDirectory, bool> filter = null, CancellationToken cancellationToken = default)
    {
      ...
      if (verify && !ObjectAttribute.Read) return Task.FromResult<IEnumerable<IDirectory>>(Array.Empty<IDirectory>());

      IEnumerable<IDirectory> dirs = directoryInfo.EnumerateDirectories().Select(dir => new FileSystemDirectory(dir, volume) as IDirectory);
        ...
        Func<IDirectory, bool> filter = null, SearchOption searchOption = default, CancellationToken cancellationToken = default)
    {
      ...
      if (verfify && !ObjectAttribute.Read) return Task.FromResult<IEnumerable<IDirectory>>(Array.Empty<IDirectory>());

      if (!search.Contains('*'))
        search = '*' + search + '*';

      IEnumerable<IDirectory> dirs = directoryInfo.EnumerateDirectories(search, searchOption).Select(dir => new FileSystemDirectory(dir, volume) as IDirectory);
        ...
        Func<IFile, bool> filter = null, CancellationToken cancellationToken = default)
    {
      ...
      if (verify && !ObjectAttribute.Read) return Task.FromResult<IEnumerable<IFile>>(Array.Empty<IFile>());

      IEnumerable<IFile> files = directoryInfo.EnumerateFiles().Select(f => new FileSystemFile(f, volume) as IFile);
        ...
        Func<IFile, bool> filter = null, SearchOption searchOption = default, CancellationToken cancellationToken = default)
    {
      ...
      IEnumerable<IFile> files = directoryInfo.EnumerateFiles(search, searchOption).Select(f => new FileSystemFile(f, volume) as IFile);
        ...
        bool verify = true, CancellationToken cancellationToken = default)
    {
      ...
      IDirectory destInfo = new FileSystemDirectory(newDest, destVolume);
        ...
        Func<IFile, bool> fileFilter = null, CancellationToken cancellationToken = default)
    {
      ...
      while (queue.Count > 0)
      {
        IDirectory currentDir = queue.Dequeue();
        model.DirectoryCount++;

        foreach (IDirectory dir in (await currentDir.GetDirectoriesAsync(verify, dirFilter, cancellationToken: cancellationToken)))
          queue.Enqueue(dir);

        foreach (IFile file in (await currentDir.GetFilesAsync(verify, fileFilter, cancellationToken: cancellationToken)))
        {
          model.FileCount++;
