  ...
  public class DefaultThumbnailBackgroundGenerator : IThumbnailBackgroundGenerator
  {
    ...
    private async Task RunVideoGeneratorAsync()
    {
      while (!_tokenSource.IsCancellationRequested)
      {
        ...
        while (_videoQueue.TryDequeue(out string nextFile))
        {
          _videoMaps.TryRemove(nextFile, out (IFile File, IFile ThumbFile, int Size, bool KeepAspectRatio) tuple);
          (IFile file, IFile tmbFile, int size, bool keepAspectRatio) = tuple;
          ...
          try
          {
            ...
            ImageWithMimeType thumb = await _videoEditor.GenerateThumbnailInBackgroundAsync(file, size, keepAspectRatio, cancellationToken: _tokenSource.Token);
    ...
    private async Task RunImageGeneratorAsync()
    {
      while (!_tokenSource.IsCancellationRequested)
      {
        ...
        while (_imageQueue.TryDequeue(out string nextFile))
        {
          _imageMaps.TryRemove(nextFile, out (IFile File, IFile ThumbFile, int Size, bool KeepAspectRatio) tuple);
          (IFile file, IFile tmbFile, int size, bool keepAspectRatio) = tuple;
          ...
          try
          {
            ...
            using (Stream fileStream = await file.OpenReadAsync(verify: false, cancellationToken: _tokenSource.Token))
            {
